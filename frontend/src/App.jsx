...
export default function App() {
  ...
  const fetchAll = useCallback(async (range = dateRange) => {
    ...
    const results = await Promise.allSettled([
      safe(apiFetch(withDates('/api/dashboard/arr'), { silent: true })),
      safe(apiFetch(withDates('/api/dashboard/clinics/active'), { silent: true })),
      safe(apiFetch(withDates('/api/dashboard/patients/total'), { silent: true })),
      safe(apiFetch(withDates('/api/dashboard/nrr'), { silent: true })),
      safe(apiFetch(withDates('/api/financials/mrr/breakdown'), { silent: true })),
      safe(apiFetch(withDates('/api/financials/churn-risk/by-region'), { silent: true })),
      safe(apiFetch('/api/alerts', { silent: true })),
      safe(apiFetch('/api/clinics', { silent: true })),
      safe(apiFetch('/api/platform/servers', { silent: true })),
      safe(apiFetch('/api/platform/background-processes', { silent: true })),
      safe(apiFetch(withDates('/api/platform/ai/costs'), { silent: true })),
      safe(apiFetch(withDates('/api/platform/ai/latency'), { silent: true })),
      safe(apiFetch(withDates('/api/platform/ai/pose-analysis/success-rate'), { silent: true })),
      safe(apiFetch(withDates('/api/analytics/apps/patients'), { silent: true })),
      safe(apiFetch(withDates('/api/analytics/apps/tablet'), { silent: true })),
      safe(apiFetch(withDates('/api/analytics/apps/web'), { silent: true })),
      safe(apiFetch(withDates('/api/analytics/features/adoption'), { silent: true })),
      safe(apiFetch(withDates('/api/analytics/adherence/global'), { silent: true })),
      safe(apiFetch(withDates('/api/analytics/retention/cohorts'), { silent: true })),
      safe(apiFetch(withDates('/api/analytics/ai/soap-quality'), { silent: true })),
      safe(apiFetch('/api/settings', { silent: true })),
      safe(apiFetch('/api/settings/azure', { silent: true })),
      safe(apiFetch('/api/settings/database', { silent: true })),
      safe(apiFetch('/api/users', { silent: true })),
      safe(apiFetch(withDates('/api/dashboard/system-health'), { silent: true })),
      safe(apiFetch(withDates('/api/dashboard/users/active-now'), { silent: true })),
      safe(apiFetch(withDates('/api/dashboard/downloads/total'), { silent: true })),
      safe(apiFetch(withDates('/api/dashboard/users/dormant'), { silent: true })),
    ]);
